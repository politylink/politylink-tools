...
def to_datetime(maybe_date_str):
    if maybe_date_str:
        return datetime.strptime(maybe_date_str, '%Y-%m-%d')
    return None
...
def main():
    ...
    maybe_start_date = to_datetime(args.start_date)
    maybe_end_date = to_datetime(args.end_date)
    news_list = gql_client.get_all_news(maybe_start_date, maybe_end_date)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--start_date', help='開始日（例: 2020-01-01）')
    parser.add_argument('-e', '--end_date', help='終了日（例: 2020-01-01）')
